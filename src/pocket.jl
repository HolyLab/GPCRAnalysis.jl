...
# Keyword arguments
- `backbone_only=false`: restrict the clash filter to backbone atoms, yielding the cavity
  the pocket would present if all side chains were absent
- `probe_radius`: solvent probe radius (Å); defaults to 1.4 (approximating a water
  molecule), or to 3.0 when `backbone_only=true` to compensate for the missing side chains
...
function detect_pocket_cavity(
    ...
    probe_radius::Real = backbone_only ? 3.0 : 1.4,
    ...
    # When backbone_only=true, side-chain atoms are excluded so that cavity geometry
    # is determined by the stable backbone scaffold rather than uncertain rotamers.
    # The larger default probe_radius (3.0 Å vs 1.4 Å) compensates by keeping probes
    # clear of the region typically occupied by Cβ/Cγ atoms.
    buffer = probe_radius + 2.2   # ≥ max heavy-atom vdW radius
    atom_coords = Vector{SVector{3,Float64}}()
    atom_radii  = Vector{Float64}()
    for r in seq
        rname = resname(r)
        for a in (backbone_only ? collectatoms(r, backboneselector) : r)
