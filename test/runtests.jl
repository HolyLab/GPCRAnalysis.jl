...
using GPCRAnalysis: percent_similarity
...
@testset "GPCRAnalysis.jl" begin
    ...
    @testset "MSA" begin
        ...
        msa = MSA.read_file(pf09645_sto, Pfam.Stockholm; generatemapping=true, useidcoordinates=true)
    ...
    @testset "Pocket residues and features" begin
        ...
        # The cavity supersedes :Steric as a description of the pocket's free volume
        @test !haskey(cp_cavity.gmms, :Steric)
        @test haskey(pp.gmms, :Steric)
        # Trimming low-amplitude features
        cp_cavity_thresh = complementary_pharmacophore(pp, cavity; ampthreshold=0.1)
        @test sum(length, values(cp_cavity_thresh.gmms)) < sum(length, values(cp_cavity.gmms))
    end
